...
def create_access_token(data: Dict, expires_delta: Optional[timedelta] = None) -> str:
    """
    Create a JWT token containing `data` dict. Example data: {"user_id": 7, "service_no": "IC12345"}
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(http_bearer)) -> Dict:
    ...
    Returns a dict (payload) that should contain at least 'user_id' and optionally 'service_no'.
    Usage:
        @router.post("/create")
